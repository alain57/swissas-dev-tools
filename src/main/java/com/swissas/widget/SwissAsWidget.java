...
public class SwissAsWidget implements ProjectComponent {
	...
	private void fillSharedProperties() {
		Module shared = Stream.of(ModuleManager.getInstance(this.project).getModules()).filter(e -> e.getName().contains("amos_shared")).findFirst().orElse(null);
		if (shared != null) {
			...
			try {
				if (propertieFile != null) { //if null then older amos release maybe add this case in future... For now don't handle it
	...
	public void projectOpened() {
		...
		if (this.swissAsStorage.isAmosProject()) {
